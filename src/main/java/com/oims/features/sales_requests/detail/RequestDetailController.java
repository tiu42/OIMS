...
public class RequestDetailController {
    ...
    public RequestDetailDTO loadRequestData() throws SQLException {
        ...
        if (requestId == null) {
            salesRequest = Optional.empty();
            throw new SQLException("Không tìm thấy mã yêu cầu.");
        }

        salesRequest = salesRequestDao.findById(requestId);
        if (salesRequest.isEmpty()) {
            throw new SQLException("Yêu cầu nhập hàng không tồn tại hoặc đã bị xóa.");
        }

        SalesRequest request = salesRequest.get();
        String id = request.getRequestId().toString();
        Map<Integer, String> creatorCache = new HashMap<>();
        String creatorName = resolveCreatorName(request.getCreatedBy(), creatorCache);
        String status = getStatusLabel(request.getStatus());
        String createdDate = request.getCreatedDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy"));
        return new RequestDetailDTO(id, creatorName, createdDate, status);
    }
    ...
    public boolean canBeginProcessing() {
        SalesRequestStatus status = getSalesRequestStatus();
        return status == SalesRequestStatus.PENDING || status == SalesRequestStatus.PROCESSING;
    }
